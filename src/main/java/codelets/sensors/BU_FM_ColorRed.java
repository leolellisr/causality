 ...
 */
public class BU_FM_ColorRed extends FeatMapCodelet {
    ...
    public BU_FM_ColorRed(SensorI vision, int nsensors, ArrayList<String> sens_names, String featmapname,int timeWin, int mapDim) {
    ...
    public void proc() {
        ...
        MemoryObject vision_bufferMO = (MemoryObject) sensor_buffers.get(0);        //Gets Red Data
        
        List visionData_buffer;
        visionData_buffer = (List) vision_bufferMO.getI();
        List vision_redFM = (List) featureMap.getI();        
        
        if(vision_redFM.size() == timeWindow){
            vision_redFM.remove(0);
        }
        ...
        ArrayList<Float> vision_redFM_t = (ArrayList<Float>) vision_redFM.get(t);
        ...
        Float Fvalue;
        float MeanValue = 0;
        ArrayList<Float> vision_mean_red = new ArrayList<>();
        ...
        for (int j = 0; j+pixel_len < visionData.size(); j+= pixel_len) {
            ...
            Fvalue = (Float) visionData.get(j);               //Gets JUST red values for VisionData

            visionData_Array.set(count_3, Fvalue);        //Red data
            count_3 += 1;
        }
        ...
        for(int n = 0;n<slices;n++){
            ...
            for(int m = 0;m<slices;m++){    
                ...
                if(correct_mean/mr>1) vision_mean_red.add(new Float(1));
                else if(correct_mean/mr<0.001) vision_mean_red.add(new Float(0));
                else vision_mean_red.add(correct_mean/mr);     
    ...
    private void printToFile(ArrayList<Float> arr){
        ...
        try(FileWriter fw = new FileWriter("profile/vision_red_FM.txt", true);
