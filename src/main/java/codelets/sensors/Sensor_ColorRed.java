 ...
 */
public class Sensor_ColorRed extends FeatMapCodelet {
    ...
    public Sensor_ColorRed(SensorI vision, int nsensors, ArrayList<String> sens_names, String featmapname,int timeWin, int mapDim) {
    ...
    public void proc() {
        ...
        for (int k = 0; k+pixel_len < visionData.size(); k += pixel_len) {
            Fvalue = (Float) visionData.get(k);                                //Gets JUST red values for VisionData
        ...
        for (int j = 0; j+pixel_len < visionData.size(); j+= pixel_len) {
            ...
            vision_redFM_t.set(count_3, Fvalue);                     //Red data
            count_3 += 1;
        }   
    ...
    private void printToFile(ArrayList<Float> arr){
        ...
        try(FileWriter fw = new FileWriter("profile/vision_red.txt", true);
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter out = new PrintWriter(bw))
        {
            out.println(dtf.format(now)+"_"+this.vision.getExp()+"_"+time_graph+" "+ arr);
            //if(time_graph == max_time_graph-1) System.out.println("vision_red: "+time_graph);
            time_graph++;
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
