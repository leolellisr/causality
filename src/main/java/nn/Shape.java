...
public class Shape {
    ...
    public static long[] getReducedShape(int[] wholeShape, int[] dimensions, boolean keepDims, boolean newFormat) {
        ...
        long[] result = ArrayUtil.toLongArray(Arrays.copyOf(wholeShape, wholeShape.length));
    ...
    public static boolean isVector(LongBuffer shapeInfo) {
        ...
        if (rank > 2 || rank < 1)
            ...
            LongBuffer shape = Shape.shapeOf(shapeInfo);
            return shape.get(0) == len || shape.get(1) == len ;
        }
    }
    ...
    public static int length(LongBuffer buffer) {
        int ret = 1;
        LongBuffer shape = Shape.shapeOf(buffer);
        int rank = Shape.rank(buffer);
        for (int i = 0; i < rank; i++)
            ret *= shape.get(i);
        return ret;
    ...
    public static int rank(LongBuffer buffer) {
        Buffer buffer2 = (Buffer) buffer;
        LongBuffer ret = (LongBuffer) buffer2.position(0);
    ...
    public static LongBuffer stride(LongBuffer buffer) {
        int rank = rank(buffer);
        Buffer buffer2 = (Buffer) buffer;
        LongBuffer ret = (LongBuffer) buffer2.position(1 + rank);
        return ret.slice();
    }
    ...
    public static LongBuffer shapeOf(LongBuffer buffer) {
        Buffer buffer2 = (Buffer) buffer;
        LongBuffer ret = (LongBuffer) buffer2.position(1);
        return ret.slice();
    }
    ...
    public static String shapeToString(LongBuffer buffer) {
        ...
        LongBuffer shapeBuff = shapeOf(buffer);
        int rank = Shape.rank(buffer);
        LongBuffer strideBuff = stride(buffer);
        StringBuilder sb = new StringBuilder();
    ...
    public static DataBuffer createShapeInformation(long[] shape, long[] stride, long elementWiseStride, char order, long extras) {
        ...
        */
        ...
        DataType dtype = ArrayOptionsHelper.dataType(extras);
        boolean empty = ArrayOptionsHelper.hasBitSet(extras, ArrayOptionsHelper.ATYPE_EMPTY_BIT);
    ...
    public static DataType pickPairwiseDataType(@NonNull DataType typeX, @NonNull DataType typeY) {
        ...
        boolean rX = isR(typeX);
        boolean rY = isR(typeY);
